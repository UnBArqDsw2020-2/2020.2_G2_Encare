...
Projetos do PMI (*Project Management Institute*) fornece o vocabulário profissional básico que pode ser utilizado uniformemente por organizações,
gerentes de projeto, de programa e de portfólio, e outras partes interessadas.  

Este glossário irá incluir terminologias gerais que são referentes a gestão de projetos que estão presentes no PMBOK 6ª Edição [[2]](#ref2), além de terminologias específicas relacionadas ao escopo do nosso projeto e metodologias adotadas, como o Scrum [[1]](#ref1).  

Vale ressaltar que o glossário do PMBOK 6ª Edição [[2]](#ref2) é mais extenso, e os termos aqui presentes foram selecionados levando em consideração o contexto do curso.

- - -  

|Termo|Termo em Inglês|Descrição|
|-----|---------------|---------|
|Abordagem de desenvolvimento| Development Approach| O método usado para criar e desenvolver o [produto](#produto), serviço ou [resultado](#resultado) durante o [ciclo de vida do projeto](#cicloVida), tal como o método de previsão, iterativo, incremental, ágil ou um método híbrido.|
|Ação corretiva| Corrective Action | Uma atividade intencional que realinha o desempenho dos trabalhos do projeto com o [plano de gerenciamento do projeto](#planoGerProjeto).|
|Ação preventiva | Preventive Action | Uma atividade intencional para garantir que o desempenho futuro do trabalho do projeto esteja alinhado com o [plano de gerenciamento do projeto](#planoGerProjeto) |
|Aceitação de risco | Risk Acceptance | Uma estratégia de resposta ao risco em que a [equipe do projeto](#equipeProjeto) decide reconhecer a existência do risco e não agir, a menos que o risco ocorra |
|Ameaça | Threat | Um risco que teria um efeito negativo em um ou mais objetivos do projeto |
|Análise de cenário E-se | What-If Scenario Analysis | O [processo ](#processo) de avaliar cenários a fim de predizer seus efeitos nos objetivos do projeto |
|Análise de [produto](#produto)| Product Analysis | Para os projetos que possuem um [produto](#produto) como entrega, é uma [ferramenta](#ferramenta) de definição do [ escopo](#escopo) que geralmente implica em fazer perguntas sobre esse [produto](#produto) e criar respostas para descrever o uso, as características e outros aspectos relevantes do que será fabricado.|
|Antecipação| Lead | A quantidade de tempo que uma atividade sucessora pode ser adiantada em relação a uma atividade predecessora|
|Atividade | Activity | Uma parte distinta e programada do trabalho executado no decorrer do projeto. |
| Atualização | Update|  Uma modificação em qualquer entrega, componente de [plano de gerenciamento do projeto](#planoGerProjeto) ou documento do projeto que não está sob controle de mudança formal.|
| <span id="productBacklog">Backlog do [produto](#produto)</span>| Product Backlog | Um artefato [Scrum](#scrum) que consiste em uma lista ordenada do trabalho a ser feito para criar, manter e sustentar um [produto](#produto). Gerenciado pelo [Product Owner](#productOwner).|
|Benchmarking | Benchmarking | Benchmarking envolve a comparação de [produtos](#produto), [processos](#processo)  e práticas reais ou planejadas com as de organizações similares para identificar as melhores práticas, gerar ideias para melhorias e fornecer uma base para medir o desempenho. |
| Calendário do projeto | Project Calendar | Um calendário que identifica os dias úteis e os turnos disponíveis para a execução das atividades agendadas. |
| <span id="cicloVida">Ciclo de vida do projeto</span> | Project Life Cycle | O ciclo de vida do projeto é a série de fases pelas quais um projeto passa, do início à conclusão. Ele fornece a estrutura básica para o gerenciamento do projeto. |
|Ciclo de vida incremental|Incremental Life Cycle|Um [ciclo de vida do projeto](#cicloVida) adaptativo no qual a entrega é produzida através de uma série de iterações que sucessivamente adicionam funcionalidade dentro de um prazo predeterminado. A entrega inclui a capacidade necessária e suficiente para ser considerada completa somente após a iteração final.|
|Ciclo de vida iterativo|Iterative Life Cycle|[Ciclo de vida do projeto](#cicloVida) em que o [escopo do projeto](#escopoProjeto) é geralmente determinado no início do ciclo de vida do mesmo, mas as estimativas de tempo e custos são rotineiramente modificadas à proporção que a compreensão do [produto](#produto) pela [equipe do projeto](#equipeProjeto) aumenta. Iterações desenvolvem o [produto](#produto) através de uma série de ciclos repetidos, enquanto os [incrementos](#incremento) sucessivamente acrescentam à funcionalidade do [produto](#produto).|
|Ciclo de vida preditivo | Predictive Life Cycle | Uma forma de [ciclo de vida do projeto](#cicloVida) na qual o [ escopo](#escopo), tempo e custo do projeto são determinados nas fases iniciais do ciclo de vida.|
|Coletar os requisitos|Collect Requirements| O [processo ](#processo) de determinar, documentar e gerenciar as necessidades e requisitos das partes interessadas a fim de atender aos objetivos do projeto.|
| <span id="conhecimento">Conhecimento</span> | Knowledge|  Uma combinação de experiências, valores e convicções, informações contextuais, intuição e insight que as pessoas usam para entender novas experiências e informações.|
|<span id="controlarQualidade">Controlar a qualidade</span>|Control Quality| O [processo ](#processo) de monitoramento e registro dos resultados da execução de atividades de gerenciamento da qualidade para avaliação de desempenho e garantir que as saídas do projeto sejam completas, corretas e atendam as expectativas do cliente.
|Controlar o cronograma | Control Schedule | O [processo ](#processo) de monitoramento do status do projeto para atualizar o cronograma do projeto e gerenciar mudanças na linha de base do cronograma|
...
|Equipe de gerenciamento do projeto | Project Management Team | Os membros da [equipe do projeto](#equipeProjeto) que estão diretamente envolvidos nas atividades de gerenciamento do projeto.|
...
|Papel | Role | Um papel definido a ser realizado por um membro da [equipe do projeto](#equipeProjeto), como testes, arquivamento, inspeção ou codificação.|
